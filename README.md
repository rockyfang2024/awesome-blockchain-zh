...
我们对比了已有的中文区块链资源仓库（如 `chaozh/awesome-blockchain-cn`），做了这些差异化：

| 维度 | 传统 awesome 仓库 | 我们 |
|---|---|---|
| **维护频率** | 经常 1+ 年没更新 | **持续维护 + 每个资源标时间戳** |
| **覆盖公链** | 偏 ETH 中心化 | **全链：ETH/Solana/BTC/L2/Aptos/Sui/Move** |
| **目标读者** | 多偏开发者 | **新手 → 进阶 → 专家** 全覆盖 |
| **视频教程** | 大多缺失 | **B 站 / YouTube 中文视频专题** |
| **入门友好** | 直接堆资源 | **快速导航表 + 入门 101 章节** |
| **信息源** | 多缺失 | **公众号 / Newsletter / Twitter / 数据平台** |
| **热点事件分析** | ❌ 多缺失 | ✅ **本仓库特色：每次大事件都有复盘 + 学习点** |
| **实战项目** | 有但没分类 | **按语言分类（Go/Rust/Solidity）+ 学习路径** |

> ⚠️ **数据时效说明**：本仓库所有数据（事件、链接、价格、TVL）都可能在快速变化。
> 任何具体数字请以原始来源为准。本仓库**只做学习资源的整理和事件的分析框架**，
> 不提供投资建议。
